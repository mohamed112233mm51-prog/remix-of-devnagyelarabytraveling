...
function buildCounterparty(
  ...
  return SOURCE_LABELS[String(split.source_table || "")] || String(split.source_table || "حركة مالية");
}
...
export function TreasuryMovementLedger({
  ...
  const ledger = useMemo(() => {
    ...
    for (const split of sortedDesc) {
      ...
      rows.push({
        ...
        counterparty: buildCounterparty(split, groupBySource, boxNameById),
  ...
  }, [allSplits, trackedBoxes, trackedIds, boxNameById, inRange]);
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={row.id}>
                  ...
                  <td data-label="المصدر / الطرف">{row.counterparty}</td>
