...
export function PassportBulkUploadWorkspaceV3() {
  ...
  const addPdf = async (file: File) => {
    ...
    try {
      ...
      try {
        ...
        const scanSuccess = results.filter(Boolean).length;
        const totalFailed = renderFailed + (results.length - scanSuccess);
        if (scanSuccess) toast.success(`تمت قراءة ${scanSuccess} جواز من PDF${totalFailed ? `، وفشل ${totalFailed}` : ""}`);
        else toast.error("تعذر قراءة أي جواز من ملف PDF");
      } finally {
  ...
  const saveSelected = async () => {
    ...
    try {
      for (const r of selected) {
        ...
        if (result.insertedIds.length === 1 && result.failed === 0) { ok++; updateRow(r.id, { state: "saved", selected: false }); }
