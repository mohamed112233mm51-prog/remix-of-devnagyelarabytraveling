...
# financialEngine: give the two transfer splits one shared reference.
p = Path("src/lib/financialEngine.ts")
...
export async function postCashBoxTransfer(args: {
  ...
  return postMovement({
    ...
    splits: [
'''
s = once(s, old, new, "financial transfer grouping")
p.write_text(s, encoding="utf-8")


# reports: import transfer helpers and explicit live refresh.
...
s = once(
    ...
    "treasury modal mount",
)

marker = 'function CashBoxReconcileModal({ box, onClose }: { box: CashBoxRow; onClose: () => void }) {'
if s.count(marker) != 1:
    raise SystemExit("reconcile marker not found exactly once")

modal = r'''function CashBoxTransferModal({ boxes, onClose }: { boxes: CashBoxRow[]; onClose: () => void }) {
